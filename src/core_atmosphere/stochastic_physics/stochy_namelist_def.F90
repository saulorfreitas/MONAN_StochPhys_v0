...
! pjp stochastic phyics
      ...
      real(kind=kind_phys), dimension(5) :: lndp_lscale,lndp_tau
      integer n_var_lndp
      integer(kind=8),dimension(5) ::iseed_lndp
      integer lndp_type
      integer lndp_model_type
      character(len=3), dimension(max_n_var_lndp)         ::  lndp_var_list
      real(kind=kind_phys), dimension(max_n_var_lndp) ::  lndp_prt_list

      real(kind=kind_phys), dimension(max_n_var_spp) :: spp_lscale    &
