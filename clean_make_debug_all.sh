...
#make -j 8 gfortran CORE=init_atmosphere DEBUG=true OPENMP=true USE_PIO2=false PRECISION=single 2>&1 | tee -a ${MAKE_OUT_FILE}
#make -j 8 intel CORE=init_atmosphere DEBUG=true  OPENMP=true USE_PIO2=false PRECISION=single 2>&1 | tee ${MAKE_OUT_FILE}
make -j 8 intel2-xd2000 CORE=init_atmosphere DEBUG=true OPENMP=true USE_PIO2=false PRECISION=single 2>&1 | tee ${MAKE_OUT_FILE}
#make -j 8 intel-xd2000 CORE=init_atmosphere DEBUG=true  OPENMP=true USE_PIO2=false PRECISION=single OPTIMIZATION_LEVEL=O1 FFLAGS_OPT=-O1 CFLAGS_OPT=-O1 CXXFLAGS_OPT=-O1 2>&1 | tee ${MAKE_OUT_FILE}

make clean CORE=atmosphere
#make -j 8 gfortran CORE=atmosphere DEBUG=true  OPENMP=true USE_PIO2=false PRECISION=single 2>&1 | tee ${MAKE_OUT_FILE}
#make -j 8 intel CORE=atmosphere DEBUG=true  OPENMP=true USE_PIO2=false PRECISION=single 2>&1 | tee ${MAKE_OUT_FILE}
make -j 8 intel-xd2000 CORE=atmosphere DEBUG=true OPENMP=true USE_PIO2=false PRECISION=single 2>&1 | tee ${MAKE_OUT_FILE}
